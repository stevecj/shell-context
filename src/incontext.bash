...
# ~/.bash_profile. It provides functions for managing your working
# context with the In Context project.
#
...
# file in the ~/.config/incontext/contexts/ directory, which is
...
# ~/.config/incontext/contexts/ directory, which is sourced near the
...
# ~/.config/incontext/contexts/ directory, which is sourced before
# switching from one context to another.
#
# If there is a file named _default.context-start in the
# ~/.config/incontext/contexts/ directory, then it will be sourced
# when no context is loaded. Similarly, if there is a file named
# _default.context-finalize in the ~/.config/incontext/contexts/
...
if [[ ! -d "$HOME/.config/incontext/contexts" ]]; then
  mkdir -p "$HOME/.config/incontext/contexts"
fi

function _incontext-init-usage() {
  cat <<EOF
Usage: incontext-init-start
Usage: incontext-init-start -h
...
function incontext-init-start() {
  ...
  if [[ -n "$INCONTEXT" ]]; then
    ...
    if [[ -f "$HOME/.config/incontext/contexts/_default.context-start" ]]; then
      export INCONTEXT_START_FILE="$HOME/.config/incontext/contexts/_default.context-start"
      . "$INCONTEXT_START_FILE"
    fi
  fi
}
...
function _incontext-finalize-usage() {
  cat <<EOF
Usage: incontext-init-finalize
Usage: incontext-init-finalize -h
...
function incontext-init-finalize() {
  local opt OPTARG
  while getopts ":h" opt; do
    case $opt in
  ...
  elif [[ -f "$HOME/.config/incontext/contexts/_default.context-finalize" ]]; then
    . "$HOME/.config/incontext/contexts/_default.context-finalize"
  fi
}

function _incontext-in-prompt-title-usage() {
  cat <<EOF
Usage: incontext-in-prompt-title [format] [default_value]
Usage: incontext-in-prompt-title -h
...
function incontext-in-prompt-title() {
...
function _use-incontext-usage() {
  cat <<EOF
Usage: use-incontext <context_name>
Usage: use-incontext -h
...
Arguments:
  context_name:
    ...
    ~/.config/incontext/contexts/ directory.
...
function use-incontext() {
  ...
  local context_start_file=$HOME/.config/incontext/contexts/"$context_name".context-start
  if [[ ! -f $context_start_file ]]; then
    echo "No context-start file found for '$context_name' at $context_start_file." >&2
    return 1
  fi

  local context_finalize_file=$HOME/.config/incontext/contexts/"$context_name".context-finalize
...
function _unload-incontext-usage() {
  cat <<EOF
Usage: unload-incontext [-qy]
Usage: unload-incontext -h
...
function _use-local-incontext-usage() {
  cat <<EOF
Usage: use-local-incontext [options]
Usage: use-local-incontext -h
...
.incontext file is found. See the help for use-incontext for more
...
function use-local-incontext() {
  ...
  if [[ -n "$context_file" ]]; then
    ...
    if [[ -n "$INCONTEXT" ]]; then
      ...
      unload-incontext -y ${be_less_verbose:+-q}
    else
      if [[ $be_less_verbose != 1 ]]; then
